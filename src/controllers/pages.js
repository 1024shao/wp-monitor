...
class pages {
  ...
  async getPageList(ctx) {
    try {
      ...
      console.log('sqlstr: getlist **', sqlstr)
  ...
  async getPageItemDetail(ctx) {
    try {
      ...
      // 获得列表
      let sqlstr = sql.table('web_pages').where(data).order('createTime desc').page(pageNo, pageSize).select()
      let result = await mysql(sqlstr)
  ...
  async getPageItemForId(ctx) {
    try {
      ...
      // 获得列表
      let sqlstr = sql.table('web_pages').where({ id: id }).select()
      let result = await mysql(sqlstr)
